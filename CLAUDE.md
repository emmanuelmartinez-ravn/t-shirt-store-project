...
- **Always verify which branch you're on (`git branch --show-current` or `git status`) before making or committing changes**, and check for uncommitted work before switching branches (stash it rather than discarding it). Don't assume the checked-out branch matches the task at hand.
- **`claude-refactoring` is a long-lived branch reserved for upgrades/refactoring only — not new features.** While working on `claude-refactoring` (or any branch created from it), do NOT open a PR into `main` by default, even if asked to run `/pr` or push changes — confirm explicitly with the user first before targeting `main`. See `.claude/skills/pr/SKILL.md`.
...
- `.claude/skills/commit/SKILL.md` (`/commit`) and `.claude/skills/pr/SKILL.md` (`/pr`) — this repo's commit/PR workflow; prefer these over ad hoc git commands.
