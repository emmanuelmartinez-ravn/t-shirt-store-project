...
8. Push the branch to its remote: `git push -u origin <branch>` if no upstream is tracked yet, otherwise `git push`. If the current branch is `main`, skip this and tell the user instead — pushing directly to `main` bypasses review and needs their explicit confirmation first, same as `/pr.md`'s handling of `main`.
